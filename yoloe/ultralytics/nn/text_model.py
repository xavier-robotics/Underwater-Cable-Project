...
class MobileCLIP(TextModel):
    ...
    @staticmethod
    def resolve_pretrained(size):
        filename = f"mobileclip_{size}.pt"
        candidates = [
            Path(os.environ.get("YOLOE_CKPT_DIR", "")) / filename if os.environ.get("YOLOE_CKPT_DIR") else None,
            Path.cwd() / filename,
            Path(__file__).resolve().parents[3] / "ckpts" / "yoloe" / filename,
    ...
    def __init__(self, size, device):
        ...
        pretrained = self.resolve_pretrained(size)
